...
def test_737_linear_chain_single_cap():
    plan = MachinePlan.linear_chain(["cap:only"], "media:pdf", "media:png", ["source_file"])
...
def test_738_linear_chain_empty():
    plan = MachinePlan.linear_chain([], "media:pdf", "media:png", [])
...
def test_744_plan_serialization():
    plan = MachinePlan.single_cap("cap:test", "media:pdf", "media:png", "input_file")
...
def test_749_get_node():
    plan = MachinePlan.single_cap("cap:test", "media:pdf", "media:png", "doc_path")
...
def test_935_find_first_foreach_linear():
    plan = MachinePlan.linear_chain(["cap:a", "cap:b"], "media:pdf", "media:png", ["input_a", "input_b"])
...
def test_936_has_foreach():
    ...
    linear_plan = MachinePlan.linear_chain(["cap:a"], "media:pdf", "media:png", ["input_a"])
    ...
    standalone_collect_plan.add_node(
        MachineNode.cap("cap_0", "cap:in=media:textable;op=summarize;out=media:summary")
    )
