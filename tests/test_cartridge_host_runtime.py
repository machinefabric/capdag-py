...
def test_8116_released_rid_ring_discriminates_dedupes_and_ages_out():
    ...
    assert not runtime.recently_released_rid(rid), "nothing released yet"
    ...
    assert len(runtime.recent_released_rids) == 1, \
        "duplicate releases collapse to one ring entry"
    assert not runtime.recently_released_rid(MessageId(9999)), \
...
def test_8117_unroutable_continuation_classified_by_release_ring():
    ...
    assert runtime.route_continuation_frame(
        xid, MessageId(41), FrameType.CHUNK, None
    ) is None
